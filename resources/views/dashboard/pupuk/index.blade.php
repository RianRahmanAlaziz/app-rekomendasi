...
@section('container')
                ...
                <div class="card border shadow-lg mb-4">
                    <div class="card-header border-bottom pb-0">
                        <div class="d-sm-flex align-items-center">
                            <div>
                                <h6 class="font-weight-semibold text-lg mb-0">Data Pupuk</h6>
                                <p class="text-sm"></p>
                            </div>
                            <div class="ms-auto d-flex">
                                <button type="button" class="btn btn-sm btn-dark btn-icon d-flex align-items-center me-2"
                                    ...
                                    <span class="btn-inner--text">Add Data Pupuk</span>
                                </button>
                            </div>
                        </div>
                    </div>
                    <div class="card-body px-0 py-0">
                        ...
                        <div class="table-responsive p-0">
                            <table class="table table-hover align-items-center mb-0">
                                ...
                                <tbody>
                                    @forelse ($pupuks as $item)
                                        <tr>
                                            <td class="text-center align-middle text-secondary text-sm font-weight-normal">
                                                {{ ($pupuks->currentPage() - 1) * $pupuks->perPage() + $loop->iteration }}
                                            </td>
                                            <td>
                                                <div class="d-flex px-2 py-1">
                                                    <div class="d-flex align-items-center">
                                                        <img src="/assets/img/pupuk/{{ $item->gambar }}"
                                                            class="avatar avatar-sm rounded-circle me-2" alt="user1">
                                                    </div>
                                                    <div class="d-flex flex-column justify-content-center ms-1">
                                                        <h6 class="mb-0 text-secondary text-sm font-weight-semibold">
                                                            {{ $item->nama }}</h6>
                                                    </div>
                                                </div>
                                            </td>
                        ...
                        <div class="border-top py-3 px-3 d-flex align-items-center">
                            <p class="font-weight-semibold mb-0 text-dark text-sm">
                                Page {{ $pupuks->currentPage() }} of {{ $pupuks->lastPage() }}
                            </p>
                            <div class="ms-auto">
                                @if ($pupuks->onFirstPage())
                                    <button class="btn btn-sm btn-white mb-0" disabled>Previous</button>
                                @else
                                    <a href="{{ $pupuks->previousPageUrl() }}"
                                        class="btn btn-sm btn-white mb-0">Previous</a>
                                @endif

                                @if ($pupuks->hasMorePages())
                                    <a href="{{ $pupuks->nextPageUrl() }}" class="btn btn-sm btn-white mb-0">Next</a>
                                @else
            ...
            <div class="modal-dialog ">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <form action="/dashboard/data-pupuk/{{ $item->id }}" method="POST">
                            @method('DELETE')
                            @csrf
    ...
    @include('dashboard.pupuk.add')
    @include('dashboard.pupuk.edit')
@endsection
